    ...
    void Update()
    {
      ...
      m_IsPaused = superhotScript.babaMode;
        ...
        if (!m_IsPaused && !LockControl)
        {
            ...
            if(!ruleHandler.CheckEffect("You is Stop")) // Controlled movement
            {
                move = new Vector3(Input.GetAxisRaw("Horizontal"), 0, Input.GetAxisRaw("Vertical"));
            }

            if(ruleHandler.CheckEffect("You is Move")) // Forced movement -- add direction later
            {
            ...
            move = move * usedSpeed * Time.fixedDeltaTime;
            ...
            Speed = move.magnitude / (PlayerSpeed * Time.fixedDeltaTime);
